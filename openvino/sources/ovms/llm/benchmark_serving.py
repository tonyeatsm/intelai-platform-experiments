...
def load_local_vision_arena_dataset(dataset_root: str, dataset_split: Optional[str]):
    data_dir = os.path.join(dataset_root, "data")
    if not dataset_split:
        dataset_split = "train"
    if not os.path.isdir(data_dir):
        raise FileNotFoundError(f"Expected '{data_dir}' to be a directory.")

    data_files = [
        os.path.join(data_dir, file_name) for file_name in os.listdir(data_dir)
...
def sample_hf_requests(
    ...
    is_local_vision_arena = (
        dataset_path is not None
        and os.path.isdir(dataset_path)
        and os.path.basename(os.path.normpath(dataset_path)) ==
        "vision-arena-bench-v0.1"
        and dataset_subset is None)
    if dataset_path == 'lmarena-ai/vision-arena-bench-v0.1' \
        and dataset_subset is None or is_local_vision_arena:
        assert (dataset_split or "train") == "train"
        if is_local_vision_arena:
            dataset = load_local_vision_arena_dataset(dataset_path,
                                                      dataset_split)
        else:
